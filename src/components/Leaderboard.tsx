      ...
      <div className="bg-white dark:bg-black px-5 pb-10 pt-5 rounded-md">
        ...
        {loading ? (
          ...
          <table className="table-auto">
            ...
            <tbody>
              {search.length > 0 ? filteredScores.filter((score: Scoreboard) => score.difficulty === difficulty).slice(0, 10).map((score: Scoreboard, index: number) => (
                <tr key={index}>
              ...
              scores.filter((score: Scoreboard) => score.difficulty === difficulty).slice(0, 10).map((score: Scoreboard, index: number) => (
                <tr key={index}>
                  <td className="border px-4 py-2 text-center">{score.rank}</td>
                  <td className="border px-4 py-2">{score.name}</td>
                  <td className="border px-4 py-2">{formatTime(score.time)}</td>
